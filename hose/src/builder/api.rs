...
impl TxBuilder {
    ...
    fn mint_or_burn_asset(
        ...
        let has_policy_mint = self.body.mint.keys().any(|id| id.policy == asset_id.policy);
        if has_policy_mint {
    ...
    /// the result will be the empty interval, leading to a InvalidValidityInterval error.
    ///
    /// Example:
    /// ```
